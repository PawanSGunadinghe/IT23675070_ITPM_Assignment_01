...
DEFAULT_EXCEL_CANDIDATES = [
    str(TESTS_DIR / "IT23675070_Assignment 1 - Test cases.xlsx"),
]
...
def _find_column_index(header_values: list, requested_name: str | None, candidates: list[str]) -> int | None:
    norm_headers = [(_normalize_header(v), i) for i, v in enumerate(header_values, start=1) if v is not None]

    # 1. Try exact match for requested name
    if requested_name:
        target = _normalize_header(requested_name)
        for nh, i in norm_headers:
            if nh == target:
                return i

    # 2. Try exact match for candidates
    for cand in candidates:
        target = _normalize_header(cand)
        for nh, i in norm_headers:
            if nh == target:
                return i
...
def _wait_for_button_ready(page, action_locator, timeout_ms: int):
    if not action_locator:
        return
    try:
        # Wait for the button text to change back from "Transliterating..." to "Transliterate"
        deadline = time.time() + (timeout_ms / 1000)
        while time.time() < deadline:
            text = action_locator.inner_text()
            if "Transliterating" not in text:
                return
...
def run_test():
    ...
    # Clear existing results in these columns
    print("Clearing existing results...")
    for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
        _set_cell_value(ws, row_index, actual_col_idx, None)
        _set_cell_value(ws, row_index, status_col_idx, None)
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            if not _is_top_left_of_merged_cell(ws, row_index, input_col_idx):
                _set_cell_value(ws, row_index, status_col_idx, "N/A")
                continue
            ...
            if not singlish_input:
                _set_cell_value(ws, row_index, status_col_idx, "No Input")
                continue
            ...
            try:
                print(f"Testing [Row {row_index}]: {singlish_input[:50]}...")
            except Exception:
            ...
            try:
                ...
                if action_locator:
                    try:
                        ...
                        # Wait for "Transliterating..." to finish
                        _wait_for_button_ready(page, action_locator, 60000)
                    except Exception:
                        pass

                page.wait_for_timeout(max(0, int(args.wait_ms)))
                
                # Wait for visible content - retry more times and wait longer
                actual_output = ""
                ...
                for i in range(tries):
                    ...
                    if not current:
                        ...
                        continue
                    # If it's the same as previous, it might be still loading or it might be the same result
                    if prev_output and current == prev_output:
                        # Give it some more time, but eventually accept it if it persists
                        if i < tries // 2:
                            page.wait_for_timeout(max(0, int(args.retry_wait_ms)))
                            continue
                ...
                try:
                    _set_cell_value(ws, row_index, actual_col_idx, "Error")
                    _set_cell_value(ws, row_index, status_col_idx, "UI Error")
                except Exception:
                    pass
                # Optional: refresh page if multiple errors occur
                if processed % 5 == 0:
                    try:
                        page.reload()
